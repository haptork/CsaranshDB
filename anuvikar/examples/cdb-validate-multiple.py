...
def validateArchive(srcDir, extractionDir, metaFiles, overwriteJson, overwriteDb):
  config = getDefaultConfig("info") # check bottom cell for various keys / options to configure
...
if __name__ == "__main__":
    if len(sys.argv) < 4:
        print("please provide srcdir, extractiondir, overwriteJson, overwriteDb, metaFiles")
    else:
        srcDir = sys.argv[1]
        extractionDir = sys.argv[2]
        overwriteJson = (sys.argv[3] == "1")
        overwriteDb = (sys.argv[4] == "1")
        metaFiles = [x for x in sys.argv[5:]]
