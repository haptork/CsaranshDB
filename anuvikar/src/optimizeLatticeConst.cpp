...
auto optimizeForOffset(std::vector<av::Coords> &atoms, double minLatConst,
  ...
  for (double curLatConst = minLatConst; curLatConst < maxLatConst;
       curLatConst += step) {
    double res = 0.0;
    av::AddOffset addOffset{curLatConst, "bcc",
                                  av::Coords{{0., 0., 0.}}};
    ...
    auto atomsToIgnore = av::atomsToIgnore;
    ...
    if (toConsider < av::atomsToIgnore) toConsider = allOffsets.size();
    std::nth_element(
...
auto optimizeLatConst(std::vector<av::Coords> &atoms, double latConst) {
