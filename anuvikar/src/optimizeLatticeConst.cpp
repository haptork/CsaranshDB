...
auto optimizeForOffset(std::vector<avi::Coords> &atoms, double minLatConst,
  ...
  for (double curLatConst = minLatConst; curLatConst < maxLatConst;
       curLatConst += step) {
    double res = 0.0;
    avi::AddOffset addOffset{curLatConst, "bcc",
                                  avi::Coords{{0., 0., 0.}}};
    ...
    auto atomsToIgnore = avi::atomsToIgnore;
    ...
    if (toConsider < avi::atomsToIgnore) toConsider = allOffsets.size();
    std::nth_element(
...
auto optimizeLatConst(std::vector<avi::Coords> &atoms, double latConst) {
